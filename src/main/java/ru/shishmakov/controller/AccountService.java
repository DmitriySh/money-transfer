...
public class AccountService {
    ...
    public Optional<Account> withdraw(long number, BigDecimal amount) {
        try {
            checkAmount(amount);
            Account account = repository.findByAccNumberAndLock(number)
    ...
    @Transactional(rollbackFor = Exception.class)
    public Optional<Account> deposit(long number, BigDecimal amount) {
        try {
            checkAmount(amount);
            Account account = repository.findByAccNumberAndLock(number)
                    .orElseThrow(() -> new IllegalArgumentException("not found id: " + number));
    ...
    @Transactional(rollbackFor = Exception.class)
    public Optional<List<Account>> transfer(long from, long to, BigDecimal amount) {
        try {
            checkAmount(amount);
            ...
            return Optional.of(repository.saveAll(List.of(firstAccount, secondAccount)));
        } catch (Exception e) {
